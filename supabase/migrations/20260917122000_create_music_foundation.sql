...
create index if not exists profiles_profile_music_track_idx
  on public.profiles (profile_music_track_id)
  where profile_music_track_id is not null;

create index if not exists dumps_music_track_idx
  on public.dumps (music_track_id)
...
for each row
execute function public.set_music_track_updated_at();
...
create policy "Authenticated users can read music licenses"
on public.music_licenses
for select
to authenticated
using (true);

drop policy if exists "Authenticated users can read active approved music" on public.music_tracks;
create policy "Authenticated users can read active approved music"
