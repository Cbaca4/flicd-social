...
as $$
begin
  if new.profile_music_track_id is not null then
    if not exists (
      select 1
      from public.music_tracks
      where id = new.profile_music_track_id
        and active = true
        and approved = true
    ) then
...
for each row
execute function public.validate_music_track_assignment();
...
for each row
execute function public.validate_dump_music_track_assignment();
