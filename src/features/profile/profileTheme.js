...
function normalizeSectionStyle(value, fallback) {
  ...
  if (!next.backgroundMedia || typeof next.backgroundMedia !== "object" || !/^https?:\/\//.test(String(next.backgroundMedia.url || ""))) {
    ...
    next.backgroundMedia = {
      ...
      positionX: Number.isFinite(Number(next.backgroundMedia.positionX)) ? Math.min(100, Math.max(0, Number(next.backgroundMedia.positionX))) : 50,
      positionY: Number.isFinite(Number(next.backgroundMedia.positionY)) ? Math.min(100, Math.max(0, Number(next.backgroundMedia.positionY))) : 50,
      scale: Number.isFinite(Number(next.backgroundMedia.scale)) ? Math.min(1.6, Math.max(1, Number(next.backgroundMedia.scale))) : 1,
    };
  }
  return next;
}
...
export function sanitizeProfileTheme(input = {}) {
  ...
  button.text = DEFAULT_THEME.buttonStyle.text;
