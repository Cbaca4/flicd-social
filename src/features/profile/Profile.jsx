...
export default function Profile({ profile, activeSpace, onSwitchSpaces, theme, onCustomize, onEditProfile, boards = [], onOpenBoards, onCustomizeBoards, onOpenBoard }) {
  ...
  return (
    <div className="screen" style={{ background: theme.background }}>
      ...
      <div className="profile-layout" style={{ marginTop: 14 }}>
        ...
        <div className="stack">
          <button type="button" className="card" style={{ textAlign: "left" }} onClick={onSwitchSpaces}><div className="eyebrow">Active space</div><div className="row" style={{ marginTop: 7 }}><div style={{ flex: 1 }}><strong>@{activeSpace.handle}</strong><p className="subtitle">Switch between your identities.</p></div><ChevronRight size={18} className="muted" /></div></button>
          <div className="card"><div className="eyebrow">Profile Studio</div><h3 style={{ marginTop: 5 }}>Build your page your way</h3><p className="subtitle" style={{ marginTop: 5 }}>Nostalgic customization, with modern controls and privacy intact.</p><button type="button" className="btn btn-primary" style={{ marginTop: 12 }} onClick={onCustomize}><Settings size={15} />Customize profile</button></div>
        </div>
      </div>
    </div>
  );
