...
export default function SeasonalOverlay() {
  ...
  return <div className={`seasonal-overlay seasonal-${event.id}`} aria-label={`${event.label} seasonal decoration`}>
    {effect === "snow" && <div className="seasonal-snow" aria-hidden="true">{SNOW.map((flake, index) => <i key={index} style={flake} />)}</div>}
