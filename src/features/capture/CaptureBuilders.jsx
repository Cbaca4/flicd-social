...
export function RollBuilder({spaces,activeSpaceId,onCancel,onPost}){
  const [frames,setFrames]=React.useState(8);const [shot,setShot]=React.useState(0);const [expiry,setExpiry]=React.useState('once');const [postAs,setPostAs]=React.useState(activeSpaceId);const [developing,setDeveloping]=React.useState(false);
  if(developing)return <div className="screen"><RollDeveloping frameCount={frames} onComplete={()=>setDeveloping(false)}/><div className="row" style={{justifyContent:'center',marginTop:16}}><button className="btn" onClick={onCancel}>Cancel</button><button className="btn btn-primary" onClick={()=>onPost({mood:'roll',expiry,channel:postAs,frameCount:frames})}>Post roll</button></div></div>;
