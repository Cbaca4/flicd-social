...
export function RollBuilder({ activeSpaceId, onCancel, onPost }) {
  ...
  return <div className="screen"><div className="topbar"><div><div className="eyebrow">New roll</div><h1 className="title">Disposable camera energy</h1></div><button className="btn" onClick={onCancel}>Cancel</button></div>{shot === 0 ? <div className="stack"><div className="card"><p className="subtitle">Pick how many frames you want. Each frame is captured from an image file and cannot be deleted or reordered once captured.</p><div className="wrap" style={{ marginTop: 14 }}>{[8, 12, 24].map(n => <Pill key={n} active={frames === n} onClick={() => setFrames(n)}>{n} frames</Pill>)}</div></div><button className="btn btn-primary" onClick={() => { setCapturedFiles([]); setShot(1); }}>Start roll</button></div> : <div className="stack"><input ref={captureInputRef} type="file" accept={MEDIA_ACCEPT} capture="environment" onChange={handleCapture} style={{ display: 'none' }} /><div className="row" style={{ justifyContent: 'space-between' }}><span className="eyebrow">Frames</span><span className="flicd-mono">{capturedFiles.length}/{frames}</span></div><div className="grid grid-3">{Array.from({ length: frames }).map((_, i) => <div key={i} className="board-cover" style={{ opacity: i < capturedFiles.length ? .95 : .28 }}><span style={{ position: 'absolute', inset: 0, display: 'grid', placeItems: 'center', padding: 8, textAlign: 'center' }}>{i < capturedFiles.length ? capturedFiles[i].name : i + 1}</span></div>)}</div>{capturedFiles.length < frames ? <button className="btn btn-cyan" onClick={handleCaptureClick}>Capture frame {capturedFiles.length + 1}</button> : <button className="btn btn-cyan" onClick={() => setDeveloping(true)}>Develop roll</button>}{error && <p role="alert" className="subtitle">{error}</p>}{capturedFiles.length === frames && <div className="card"><p className="eyebrow">Expires</p><div className="wrap" style={{ marginTop: 8 }}><Pill active={expiry === '24h'} onClick={() => setExpiry('24h')}>24 hours</Pill><Pill active={expiry === 'once'} onClick={() => setExpiry('once')}>View once</Pill></div></div>}</div>}</div>;
}
