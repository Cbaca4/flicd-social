...
function FileSummary({ files }) {
  if (!files.length) {
    return <p className="subtitle" style={{ marginTop: 10 }}>Choose the moments you want to share. JPEG, PNG, or WebP · 10 MB max each.</p>;
  }
...
export function DumpBuilder({ spaces, activeSpaceId, onCancel, onPost }) {
  const [files, setFiles] = React.useState([]);
  const [mood, setMood] = React.useState('golden hour');
  const [expiry, setExpiry] = React.useState('24h');
  const [postAs, setPostAs] = React.useState(activeSpaceId);
  ...
  const post = async () => {
    ...
    try {
      ...
      await onPost({
        ...
        expiry,
        channel: postAs,
        items: files.map((file, index) => ({ note: index === 0 ? note : '', imageFile: file })),
      });
    } catch (postError) {
  ...
  return <div className="screen"><div className="topbar"><div><div className="eyebrow">New dump</div><h1 className="title">A handful of moments</h1></div><button className="btn" onClick={onCancel} disabled={posting}>Cancel</button></div><div className="stack">
    ...
    <button className="btn btn-primary" disabled={!files.length || posting} onClick={post}>{posting ? 'Uploading…' : 'Post dump'}</button>
  </div></div>;
}
...
export function RollBuilder({ spaces, activeSpaceId, onCancel, onPost }) {
  ...
  const handleCapture = (event) => {
    ...
    try {
      ...
      setShot(current => Math.min(frames, current + 1));
      setError('');
    } catch (captureError) {
      setError(captureError.message || 'Could not capture that frame.');
    }
  };

  const post = async () => {
    if (capturedFiles.length !== frames || posting) return;
    try {
      setPosting(true);
      setError('');
      await onPost({ mood: 'roll', expiry, channel: postAs, frameCount: frames, items: capturedFiles.map(file => ({ note: '', imageFile: file })) });
    } catch (postError) {
      setError(postError.message || 'Could not post roll.');
      setPosting(false);
    }
  };
  ...
  return <div className="screen"><div className="topbar"><div><div className="eyebrow">New roll</div><h1 className="title">Disposable camera energy</h1></div><button className="btn" onClick={onCancel}>Cancel</button></div>{shot === 0 ? <div className="stack"><div className="card"><p className="subtitle">Pick how many frames you want. Each frame is captured from an image file and cannot be deleted or reordered once captured.</p><div className="wrap" style={{ marginTop: 14 }}>{[8, 12, 24].map(n => <Pill key={n} active={frames === n} onClick={() => setFrames(n)}>{n} frames</Pill>)}</div></div><button className="btn btn-primary" onClick={() => { setCapturedFiles([]); setShot(1); }} >Start roll</button></div> : <div className="stack"><input ref={captureInputRef} type="file" accept={MEDIA_ACCEPT} capture="environment" onChange={handleCapture} style={{ display: 'none' }} /><div className="row" style={{ justifyContent: 'space-between' }}><span className="eyebrow">Frames</span><span className="flicd-mono">{Math.min(shot - 1, frames)}/{frames}</span></div><div className="grid grid-3">{Array.from({ length: frames }).map((_, i) => <div key={i} className="board-cover" style={{ opacity: i < capturedFiles.length ? .95 : .28 }}><span style={{ position: 'absolute', inset: 0, display: 'grid', placeItems: 'center', padding: 8, textAlign: 'center' }}>{i < capturedFiles.length ? capturedFiles[i].name : i + 1}</span></div>)}</div>{capturedFiles.length < frames ? <button className="btn btn-cyan" onClick={handleCaptureClick}>Capture frame {capturedFiles.length + 1}</button> : <button className="btn btn-cyan" onClick={() => setDeveloping(true)}>Develop roll</button>}{error && <p role="alert" className="subtitle">{error}</p>}{capturedFiles.length === frames && <div className="card"><p className="eyebrow">Expires</p><div className="wrap" style={{ marginTop: 8 }}><Pill active={expiry === '24h'} onClick={() => setExpiry('24h')}>24 hours</Pill><Pill active={expiry === 'once'} onClick={() => setExpiry('once')}>View once</Pill></div><p className="eyebrow" style={{ marginTop: 16 }}>Posting as</p><div className="wrap" style={{ marginTop: 8 }}>{spaces.map(s => <Pill key={s.id} active={postAs === s.id} onClick={() => setPostAs(s.id)}>@{s.handle}</Pill>)}</div></div>}</div>}</div>;
}
