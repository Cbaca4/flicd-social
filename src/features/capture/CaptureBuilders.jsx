...
function FileSummary({ files }) {
  if (!files.length) return <p className="subtitle" style={{ marginTop: 10 }}>Recommended feed crop: 1080×1350 (4:5). Square 1080×1080 and landscape 1080×566 are also supported. Flic'd never crops your upload. JPEG, PNG, or WebP · 20 MB max each.</p>;
...
export function DumpBuilder({ activeSpaceId, onCancel, onPost }) {
  ...
  const [allowOthersToKeep, setAllowOthersToKeep] = React.useState(true);
  ...
  const post = async () => {
    ...
    try {
      ...
      await onPost({
        ...
        location,
        taggedUsers,
        allowOthersToKeep: expiry === "24h" && allowOthersToKeep,
...
<div className="card">
  ...
  <div className="wrap" style={{ marginTop: 10 }}>
    <Pill active={expiry === "24h"} onClick={() => { setExpiry("24h"); setAllowOthersToKeep(true); }}>24 hours</Pill>
    <Pill active={expiry === "once"} onClick={() => { setExpiry("once"); setAllowOthersToKeep(false); }}>View once</Pill>
  </div>
</div><button className="btn btn-primary" disabled={!files.length || posting} onClick={post}>{posting ? 'Uploading…' : 'Post dump'}</button></div></div>;
}
...
export function RollBuilder({ activeSpaceId, onCancel, onPost }) {
  ...
  const [location, setLocation] = React.useState(null);
  const [taggedUsers, setTaggedUsers] = React.useState([]);
  const [allowOthersToKeep, setAllowOthersToKeep] = React.useState(false);
  ...
  const post = async () => {
    ...
    try {
      ...
      await onPost({
        ...
        taggedUsers,
        allowOthersToKeep: expiry === "24h" && allowOthersToKeep,
        items: capturedFiles.map(file => ({ note: '', imageFile: file })),
      });
    } catch (postError) {
