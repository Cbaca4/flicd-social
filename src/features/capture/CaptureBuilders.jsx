...
export function DumpBuilder({ activeSpaceId, onCancel, onPost }) {
  ...
  const post = async () => {
    ...
    try {
      setPosting(true);
      setError('');
      await onPost({ mood, expiry, channel: activeSpaceId, musicTrack, items: files.map((file, index) => ({ note: index === 0 ? note : '', imageFile: file })) });
    } catch (postError) {
...
export function RollBuilder({ activeSpaceId, onCancel, onPost }) {
  ...
  const post = async () => { if (capturedFiles.length !== frames || posting) return; try { setPosting(true); setError(''); await onPost({ mood: 'roll', expiry, channel: activeSpaceId, frameCount: frames, musicTrack, items: capturedFiles.map(file => ({ note: '', imageFile: file })) }); } catch (postError) { setError(postError.message || 'Could not post roll.'); setPosting(false); } };
